...
class App(Tk):
    def __init__(self):
        ...
        self.buildMenu = Menu(self.menubar, tearoff=False)
        self.menubar.add_cascade(menu=self.buildMenu, label="Compile")
        self.buildMenu.add_command(label= "Build Standalone", command= self.buildStandalone)
        self.buildMenu.add_command(label= "Build Onefile", command= self.buildOnefile)

        self.fileTitle = Text(self, height=self.SCREENHEIGHT/400, width=self.SCREENWIDTH, bg="#252525", foreground="#FFFFFF") #Displays name of file open.
        self.fileTitle.insert("1.0", f"{currentFile}")
        self.fileTitle.configure(state=DISABLED)
        self.fileTitle.pack(side=BOTTOM, fill=X)


        #Create command line.
        self.cmdFrame = Frame(self, height=self.SCREENHEIGHT/400, width=self.SCREENWIDTH)
        self.cmdFrame.pack(side=BOTTOM, fill=X)
        ...
        # A frame to hold the text editor and its scrollbars.
        self.editorFrame = Frame(self.mainFrame)
        self.editorFrame.pack(side=LEFT, fill=BOTH, expand=True)

        self.index = Text(self.editorFrame, width=4, wrap="none", bg="#222121", foreground="#FFFFFF", state='disabled')
        self.index.pack(side=LEFT, fill=Y)

        self.verticalScrollBar = Scrollbar(self.editorFrame, orient=VERTICAL)
        self.verticalScrollBar.pack(side=RIGHT, fill=Y)

        self.inputBox = Text(self.editorFrame, width=self.SCREENWIDTH//12, wrap="none", bg="#3B3939", foreground="#FFFFFF")
        self.inputBox.pack(side=TOP, fill=BOTH, expand=True)



        self.inputScrollBar = Scrollbar(self.editorFrame, orient=HORIZONTAL)
        self.inputScrollBar.pack(side=BOTTOM, fill=X)
        
        # Connect the scrollbars to the Text widgets.
        self.inputBox.config(xscrollcommand=self.inputScrollBar.set, yscrollcommand=self.verticalScrollBar.set)
        self.index.config(yscrollcommand=self.verticalScrollBar.set)

        self.verticalScrollBar.config(command=self.on_vertical_scroll)
        self.inputScrollBar.config(command=self.on_horizontal_scroll)


        self.inputBox.bind("<KeyRelease>", self.keyEvents)
        self.inputBox.bind("<Key>", self.undoRedo)
        self.inputBox.bind("<MouseWheel>", self.indexEditorSync)
        self.inputBox.bind("<Tab>", self.insertIndent)

        self.outputFrame = Frame(self.mainFrame, height=self.SCREENHEIGHT, width=int(self.SCREENWIDTH)//2, bg="#222121")
        self.outputFrame.pack(side=RIGHT, fill=BOTH, expand=True)
        self.outputTitle = Entry(self.outputFrame, width=int(self.SCREENWIDTH)//3, bg="#AAAAAA", foreground="#ff0000")
        self.outputTitle.pack(side=TOP, fill=X)
        self.outputBox = Text(self.outputFrame, height=self.SCREENHEIGHT//2, width=int(self.SCREENWIDTH)//2, bg="#222121", foreground="#FFFFFF")
        self.outputBox.pack(side=BOTTOM, fill=BOTH, expand=True)
    ...
    def indexEditorSync(self, event):
        self.index.yview_moveto(self.inputBox.yview()[0])

    def buildStandalone(self):
        ...
        os.system(f"python -m nuitka \"{self.pythonFile}\" --mingw64 --mode=standalone --windows-disable-console --enable-plugin=tk-inter")

    def buildOnefile(self):
        self.file = filedialog.askopenfile(defaultextension=".py",
                                             filetypes=[("Python File",".py"),
                                                         ("All Files", ".*")
                                                         ])
        ...
        os.system("pip install nuitka")
        os.system(f"python -m nuitka \"{self.pythonFile}\" --mingw64 --onefile --windows-disable-console --enable-plugin=tk-inter")
    ...
    def on_vertical_scroll(self, *args):
        self.inputBox.yview(*args)
        self.index.yview(*args)

    def on_horizontal_scroll(self, *args):
        self.inputBox.xview(*args)

    def insertIndent(self, event):
        self.inputBox.insert("insert", " " * 4)  # inserts 4 spaces
        return "break"
    ...
    def keyEvents(self, event): #general function for keyevents in inputBox widget
        ...
        self.fileTitle.configure(state=NORMAL)
        self.fileTitle.delete("1.0", END)
        self.fileTitle.insert("1.0", f"{currentFile} - Unsaved")
        fileSaved = False
        self.fileTitle.configure(state=DISABLED)
        ...
        if self.key == "F11": #fullscreen management
        ...
        indexList = ""
        for p in range(len(allLines)):
            indexList += str(p+1) + "\n"
        self.index.configure(state='normal')
        self.index.delete("1.0", END)
        self.index.insert("1.0", indexList)
    ...
    def openFile(self):
        ...
        self.fileTitle.configure(state=NORMAL)
        self.fileTitle.delete("1.0", END)
        self.fileTitle.insert("1.0", f"{self.pythonFile}")
        global currentFile
        global fileSaved
        fileSaved = True
        currentFile = self.pythonFile
        self.fileTitle.configure(state=DISABLED)
    ...
    def saveFile(self):
        ...
        self.pythonFile = str(self.file) #Getting name of Python file.
        self.replacer = ["<_io.TextIOWrapper name='", "' mode='w' encoding='cp1252'>"] #Getting rid of extra stuff to isolate the filename.
        for sub in self.replacer:
            self.pythonFile = self.pythonFile.replace(sub, "")
    ...
    def runFileCode(self):
        ...
        #will reuse this commented section later for inputs
        ...
        try:
            ...
            tempFilePath = os.path.join(tempfile.gettempdir(), "tempFile.py")
            with open(tempFilePath, "w") as self.tempCode:
                self.tempCode.write(self.rawCode)
            
            self.result = subprocess.run(["python", tempFilePath], capture_output=True, text=True) #run the temporary python file
            self.outputBox.configure(state=NORMAL)

            self.outputBox.insert("1.0", f"{self.result.stdout}")
            self.outputBox.insert("1.0", f"{self.result.stderr}")    
    
            try:
                if self.result.stderr:
                # Finding the exact line the error occured
                    self.lineNumberMatch = re.search(r'File ".*", line (\d+)', self.result.stderr)

                self.lineNumber = ""
                if self.lineNumberMatch:
                    self.lineNumber = self.lineNumberMatch.group(1)
                    print("THE LINE IS " + self.lineNumber)
                    self.inputBox.see(f"{self.lineNumber}.0") #send cursor directly to error line
            except:
                pass
    
            self.outputBox.configure(state=DISABLED)
            os.remove(tempFilePath) #delete the temporary python file   
    ...
    def onClosing(self):
        global fileSaved
        if fileSaved == False:
            if messagebox.askokcancel("Quit", "You haven't saved your file. Are you sure you want to quit?"):                                                
                self.destroy()
        else:
            self.destroy()
...
loader = Tk()
loader.iconbitmap("CielIcon.ico")
